...
function ContactFormComponent() {
  ...
  /** -----------------------------
   *  CONFETTI – useCallback
   * ------------------------------*/
  const triggerConfetti = useCallback(() => {
    const count = 200;
    const defaults: Options = { origin: { y: 0.7 } };
  ...
  /** -----------------------------
   *  HANDLE INPUT CHANGE (useCallback)
   * ------------------------------*/
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        const emailError = err as { status?: number; text?: string };

        if (emailError?.status === 412 && emailError.text?.includes("Invalid grant")) {
          setErrorMessage(
            t("error") +
              " - Problem z autoryzacją Gmail. Wejdź do EmailJS i kliknij Reconnect."
          );
        } else if (emailError?.status === 412) {
